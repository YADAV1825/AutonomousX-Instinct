...
**[autonomousX/Instinct-1-300m on Hugging Face](https://huggingface.co/autonomousX)**

*Instinct-1-300m is a fully reproducible, from-scratch trained 300m parameter language model trained on 30B tokens using TPU v4 infrastructure.*

**Instinct-1-300m** is a 300m parameter Large Language Model built entirely from scratch under the **AutonomousX** organization. 
...
Instinct-1-300m was trained completely **from scratch** using **JAX/Flax on TPU v4-8 hardware**. 
...
**Full training pipeline repository:** [train.py](https://github.com/YADAV1825/Instinct-300m/blob/main/train.py) (Example link format)

---

### 🚀 Run Inference (Colab TPU/GPU)

The trained LLM inference script and model weights are available at: [autonomousX/Instinct-1-300m on Hugging Face](https://huggingface.co/autonomousX).

⚠️ **Disclaimer:** *This is a sample inference script originally written for the 0.5B (40B) variants. Please adjust parameters like `N_LAYERS`, `D_MODEL`, `N_HEADS`, `D_HEAD`, `D_FF`, etc., according to this specific model's architecture (300m) to run inference successfully.*
...
repo_id = "autonomousX/Instinct-1-300m"
...
# NOTE: Adjust these parameters for your specific 300m architecture!
N_LAYERS = 32
D_MODEL = 1024
N_HEADS = 16
D_HEAD = 64
...
CKPT_PATH = os.path.abspath("TPU_300m/checkpoint_0")
...
def generate(params, input_ids, max_new_tokens=30, temperature=0.9, top_k=40):
    ...
    return input_ids
# ---------------- RUN ----------------
tokenizer = AutoTokenizer.from_pretrained("autonomousX/Instinct-1-300m")
