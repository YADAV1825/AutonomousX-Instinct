...
**[autonomousX/Instinct-1-0.5B on Hugging Face](https://huggingface.co/autonomousX)**

*Instinct-1-0.5B is a fully reproducible, from-scratch trained 0.5B parameter language model trained on 150B tokens using TPU v4 infrastructure.*

**Instinct-1-0.5B** is a 0.5B parameter Large Language Model built entirely from scratch under the **AutonomousX** organization. 
...
**Full training pipeline repository:** [train.py](https://github.com/YADAV1825/Instinct-0.5B/blob/main/train.py) (Example link format)

---

### 🚀 Run Inference (Colab TPU/GPU)

The trained LLM inference script and model weights are available at: [autonomousX/Instinct-1-0.5B on Hugging Face](https://huggingface.co/autonomousX).

⚠️ **Disclaimer:** *This is a sample inference script originally written for the 0.5B (40B) variants. Please adjust parameters like `N_LAYERS`, `D_MODEL`, `N_HEADS`, `D_HEAD`, `D_FF`, etc., according to this specific model's architecture (0.5B) to run inference successfully.*
...
# NOTE: Adjust these parameters for your specific 0.5B architecture!
N_LAYERS = 32
D_MODEL = 1024
N_HEADS = 16
D_HEAD = 64
...
CKPT_PATH = os.path.abspath("TPU_0.5B/checkpoint_0")
